    ...
    >
      <div className="rounded-2xl border border-white/[0.08] bg-neutral-900/95 shadow-2xl ring-1 ring-white/[0.06] backdrop-blur-xl">
        <div className="flex flex-wrap items-center gap-1.5 p-2 sm:gap-2 sm:p-2.5">
          ...
          {/* Language toggle — abbreviated on mobile */}
          <div className="flex rounded-xl bg-white/5 p-0.5">
            <motion.button
              type="button"
              onClick={() => setVoiceLang("en")}
              whileTap={{ scale: 0.97 }}
              className={`min-w-[2.25rem] rounded-lg px-2 py-1.5 text-xs font-medium transition-colors sm:min-w-0 sm:px-2.5 ${
        ...
        {currentList.length > 1 && (
          <div className="border-t border-white/10 px-2 pb-2 pt-1.5">
            <motion.button
              type="button"
              onClick={() => setOpen((o) => !o)}
              className="max-w-full truncate text-xs text-neutral-400 transition-colors hover:text-white"
              whileTap={{ scale: 0.98 }}
            >
            ...
            <AnimatePresence>
              {open && (
                ...
                >
                  <div className="max-h-28 overflow-y-auto rounded-xl bg-white/5 py-1 sm:max-h-32">
                    {currentList.map((v) => (
                      <li key={v.id}>
                        <motion.button
                          type="button"
